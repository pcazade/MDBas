...
void read_CONF(IO *inout,PARAM *param,ATOM **atom,double **x,double **y, double **z)
{
    ...
    *z=(double*)my_malloc(param->nAtom*sizeof(double));
    ...
    for(i=0; i<param->nAtom; i++)
    {
        ...
        strncpy((*atom)[i].label,atl,4);
        strncpy((*atom)[i].resn,ren,4);
        strncpy((*atom)[i].segn,sen,4);
...
//   int i;
    ...
    /*   for(int i=0; i<param->nAtom; i++)
       {
           ret=fwrite(atom[i].label,sizeof(char),5,inout->restFile);
           if(ret!=(size_t)5) my_error(501,__FILE__,__LINE__,0);

           ret=fwrite(atom[i].segn,sizeof(char),5,inout->restFile);
           if(ret!=(size_t)5) my_error(501,__FILE__,__LINE__,0);

           ret=fwrite(atom[i].resn,sizeof(char),5,inout->restFile);
           if(ret!=(size_t)5) my_error(501,__FILE__,__LINE__,0);

           ret=fwrite(&(atom[i].type),sizeof(int),1,inout->restFile);
           if(ret!=(size_t)1) my_error(501,__FILE__,__LINE__,0);

           ret=fwrite(&(atom[i].resi),sizeof(int),1,inout->restFile);
           if(ret!=(size_t)1) my_error(501,__FILE__,__LINE__,0);

           ret=fwrite(&(atom[i].ires),sizeof(int),1,inout->restFile);
           if(ret!=(size_t)1) my_error(501,__FILE__,__LINE__,0);
       } */
